...
def compute_mutual_info_upper_bound(Hc, P_tx, K):
    """
    计算互信息的upper-bound
    公式: R_UB^HSNR = log₂ det(H_c H_c^H p̄) - K log₂( (e Γ(K+1)^{1/K}) / (K+1) )
    其中 p̄ = P_tx / K
    ...
    # 检查维度
    if Hc.shape[0] != Hc.shape[1]:
        raise ValueError("Hc应该是方阵！")
    ...
    matrix_to_det = H_Hermitian * p_bar

    # 计算 log₂ det(H_c H_c^H p̄)
    det_value = np.linalg.det(matrix_to_det)
    det_value = np.real(det_value)  # 取实部

    if det_value <= 0:
        print(f"警告: 行列式非正: {det_value}")
        return -np.inf

    term1 = np.log2(det_value)

    # 计算第二项: K log₂( (e Γ(K+1)^{1/K}) / (K+1) )
    gamma_K1 = gamma(K + 1)
    gamma_term = gamma_K1 ** (1 / K)  # Γ(K+1)^{1/K}
    fraction = (np.e * gamma_term) / (K + 1)
    term2 = K * np.log2(fraction)

    upper_bound = term1 - term2



    return np.real(upper_bound)